...
function renderHero(){
  const view=balancedData(),rec=MODEL[M].destinations.map(balancedRec),halo=rec.reduce((a,x)=>a+x.cross_source_halo,0),benchmark=rec.reduce((a,x)=>a+x.benchmark,0),paths=MODEL[M].channels.flatMap(s=>MODEL[M].destinations.map(d=>s!==d&&balancedEffect(balancedCell(s,d))>0)).filter(Boolean).length,v=IVALID[M].summary,label={conservative:'Validated conservative',central:'Best observational',raw:'Raw association stress'}[S];
  document.getElementById('hero').innerHTML=`${short(halo,M)} <em>${label} halo scenario</em>`;
...
function renderMatrixV2(){
  ...
  for(const s of src){
    ...
    for(const d of dst){
      ...
      const title=structural?`${s} cannot receive direct attribution; any supported effect must route off-diagonal`:ok?(self?`${signed(v)} original attribution retained on diagonal`:`${signed(v)} in ${S}; validated ${num(c.scenario_effects.conservative)}, observational ${num(c.scenario_effects.central)}, raw ${num(c.scenario_effects.raw)}`):'No halo under the selected evidence standard';
  ...
  h+='</tbody><tfoot>'+reconRow('Retained original attribution','self')+reconRow({conservative:'Validated conservative halo',central:'Best observational halo',raw:'Raw association stress'}[S],'halo')+reconRow('Unassigned / non-addressable','gap')+reconRow('20% attribution benchmark','benchmark')+'</tfoot></table>';
...
function renderPanelV3(){
  ...
  const why=structural?`${s} results cannot normally be attributed back to ${s}, so this diagonal is forced to zero. Any supported ${s} effect must appear in other destination columns.`:self?`${num(v)} remains on ${d}'s original-attribution diagonal in the selected scenario. This is an accounting anchor, not a causal estimate.`:v>0?`The selected scenario moves ${num(v)} from ${d} to ${s}; Validated is ${num(c.scenario_effects.conservative)}, Best observational is ${num(c.scenario_effects.central)}, and Raw stress is ${num(c.scenario_effects.raw)}.`:`The selected evidence standard does not reassign ${d} attribution to ${s}.`;
  ...
  document.getElementById('panel').innerHTML=`<h3>${esc(s)} &rarr; ${esc(d)}</h3><span class="verdict ${c.evidence_status==='supported'?'v-hold':v>0?'v-possible':'v-no'}">${verdict}</span><p class="why">${esc(why)}</p><div class="stats">
    ...
    <div class="stat"><div class="k">Validated / Observational / Raw</div><div class="v">${num(c.scenario_effects.conservative)} / ${num(c.scenario_effects.central)} / ${num(c.scenario_effects.raw)}</div></div>
...
function renderText(){
  ...
  const tv=TVDIAG[M];
  document.getElementById('sens').innerHTML=`<p><strong>The three scenarios use genuinely different evidence standards.</strong> Validated conservative requires every hard gate and discounts weak timing separation. Best observational admits empirical-null-corrected signals even when the hard lead test fails, then applies a soft timing penalty of 1/(1 + future/correct ratio). Raw association stress deliberately removes those safeguards to show how much attribution correlation alone would move; it is a confounding diagnostic, not an estimate.</p><div class="eq">validated = corrected effect × interval weight × hard timing separation<br>best observational = empirical-null effect × interval weight × soft timing weight<br>raw stress = positive uncorrected association, capped by destination attribution</div><p><strong>TV-specific result:</strong> the US donor-control intensity model estimates a directional ${num(tv.candidate_effect)} candidate, but publishes ${num(tv.published_effect)} because it failed ${esc(tv.failed_gates.join(', ').replaceAll('_',' '))}. TV ran every day and only in the US, so false timing patterns remain too large to identify its halo reliably.</p>`;
