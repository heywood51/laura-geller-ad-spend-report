...
The primary matrix performs destination-column reconciliation on the same 20% scenario basis. Diagonal cells retain original attribution and off-diagonal cells are halo. Source evidence is discounted continuously for uncertainty, avoiding a winner-takes-all publication gate. Television is configured as non-addressable: TV→TV is structurally zero and any supported TV effect must route to another destination. Organic, Direct, and non-addressable destination remainders remain explicitly unassigned.
