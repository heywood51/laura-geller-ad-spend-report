...
The primary matrix performs destination-column reconciliation on the same 20% scenario basis. Diagonal cells retain original attribution and off-diagonal cells are halo. Source evidence is discounted continuously for uncertainty, avoiding a winner-takes-all publication gate. For every destination with a matching source row, retained diagonal attribution + cross-source halo equals 20% of original attribution. Organic and Direct remainders remain explicitly unassigned because they have no spend-source diagonal.
